...
export function ComingSoonGate({ children }: { children: React.ReactNode }) {
    ...
    const isProtected = isFrance || isPortuguese;
    ...
    // England shows a static coming soon page (no password)
    if (isEngland) {
        return (
        ...
        <div className="min-h-screen bg-[#0c121d] flex items-center justify-center px-4 font-sans">
            <div className="max-w-md w-full text-center">
                <div className="mb-8">
                    ...
                    <h1 className="text-4xl font-serif font-bold text-white mb-3">Coming Soon</h1>
                    <p className="text-gray-400 text-lg leading-relaxed">
                        {isFrance ? 'DPE France is launching soon in France.' : 'Certificado Energético is launching soon in Portugal.'}
                        <br />
                        <span className="text-[#9ACD32]">Stay tuned for updates.</span>
                    </p>
                </div>
                ...
                <p className="mt-8 text-gray-600 text-sm">
                    Contact us at <a href={`mailto:${isFrance ? 'contact@dpefrance.eu' : 'contact@certificadopt.eu'}`} className="text-[#9ACD32] hover:text-white transition">{isFrance ? 'contact@dpefrance.eu' : 'contact@certificadopt.eu'}</a> for early access.
                </p>
            </div>
        </div>
    );
