export const generateHomeownerQuoteEmail = (customerName: string, websiteUrl: string = "https://theberman.eu", promoHtml: string = "", tenant: string = 'ireland') => {
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; background-color: #f4f7f4;">
    <div style="max-width: 600px; margin: 20px auto; background-color: #ffffff; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
        <div style="background-color: #007F00; color: white; padding: 40px 20px; text-align: center;">
            ...
            <h1 style="margin: 0; font-size: 28px; font-weight: 800; letter-spacing: -0.5px;">${isSpanish ? 'Has Recibido un Nuevo Presupuesto' : 'New BER Quote Received'}</h1>
        </div>

        <div style="padding: 40px 30px;">
            <p style="font-size: 18px; font-weight: 600; margin-bottom: 20px; color: #1a1a1a;">${isSpanish ? 'Hola' : 'Hi'} ${customerName},</p>

            <p style="font-size: 16px; color: #444; margin-bottom: 25px;">
                ${isSpanish ? '¡Buenas noticias! Un certificador energético local ha enviado un presupuesto profesional para tu propiedad.' : 'Great news! A local BER Assessor has submitted a professional quote for your property assessment.'}
            </p>
            ...
            <div style="background-color: #f9fff9; border: 1px solid #d4edda; padding: 20px; border-radius: 8px; margin-bottom: 30px;">
                <p style="margin: 0; font-size: 15px; color: #155724; line-height: 1.5;">
                    <strong>${isSpanish ? 'Precio Transparente:' : 'Transparent Pricing:'}</strong> ${isSpanish ? 'Este presupuesto incluye todos los honorarios aplicables. El precio que ves es el precio final, sin extras ocultos.' : 'This quote includes all applicable SEAI fees. The price you see is the final price you pay—no hidden extras.'}
                </p>
            </div>
            ...
            <div style="text-align: center; margin: 40px 0;">
                <a href="${dashboardUrl}" style="background-color: #007F00; color: #ffffff !important; padding: 18px 40px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 18px; display: inline-block; box-shadow: 0 4px 12px rgba(0,127,0,0.2);">
                    ${isSpanish ? 'Revisar Detalles del Presupuesto' : 'Review Quote Details'}
                </a>
            </div>

            <p style="font-size: 15px; color: #555; margin-bottom: 25px;">
                ${isSpanish ? 'Puedes confirmar tu reserva al instante aceptando el presupuesto online. Un pequeño depósito asegura tu plaza en la agenda del certificador.' : "You can instantly confirm your booking by accepting the quote online. A small deposit secures your spot in the assessor's schedule."}
            </p>

            <div style="font-size: 13px; color: #666; line-height: 1.5; border-top: 1px solid #eee; padding-top: 25px; font-style: italic;">
                <strong>${isSpanish ? 'Garantía de Tranquilidad:' : 'Peace of Mind Guarantee:'}</strong> ${isSpanish ? 'Ofrecemos una garantía de devolución del 100%. Si necesitas cancelar por cualquier motivo antes de la visita, te reembolsaremos el depósito íntegro.' : 'We offer a 100% money-back guarantee. If you need to cancel for any reason before the site visit, your deposit will be refunded in full.'}
            </div>
        </div>
